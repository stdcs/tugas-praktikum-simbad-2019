...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 13, 2019 at 12:33 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_employee` (`id` INT)  SELECT * FROM employees WHERE employeeNumber = id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `get_employees` ()  SELECT * FROM employees$$
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vorder`  AS  select `customers`.`customerName` AS `CUSTOMER`,count(`products`.`productName`) AS `QTY`,group_concat('[',`products`.`productName`,']' separator ' ') AS `PRODUCTS` from (((`customers` join `orders` on(`customers`.`customerNumber` = `orders`.`customerNumber`)) join `orderdetails` on(`orders`.`orderNumber` = `orderdetails`.`orderNumber`)) join `products` on(`orderdetails`.`productCode` = `products`.`productCode`)) group by `customers`.`customerName` order by count(`products`.`productName`) ;
